...
class Administrator extends AdminController
{
     ...
     * @apiParam (入参) {String} channel_id 1 全部 2 未分配通道 3已分配
     * @apiParam (入参) {String} page 页码 默认1
     * @apiParam (入参) {String} pageNum 条数 默认10
     * @apiSuccess (返回) {String} code 200:成功 / 3001:id格式错误 
    ...
    public function getUserSendTask()
    {
        ...
        $channel_id  = is_numeric($channel_id) ? $channel_id : 1;
        intval($page);
        intval($pageNum);
        intval($free_trial);
        intval($free_trial);
        intval($channel_id);
        $result = $this->app->administrator->getUserSendTask($page, $pageNum, $id, $free_trial, $send_status, $uid, $channel_id);
        return $result;
    }
    ...
    public function setUserAccountForCmpp()
    {
        ...
        if (empty($business_id) || !is_numeric($business_id) || $business_id < 1) {
            return  ['code' => '3005', 'msg' => 'business_id格式错误'];
        }
        if (empty($cmpp_name)) {
