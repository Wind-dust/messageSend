...
class ServerSocket extends Pzlife
{
                ...
                if ($headData != false) {
                    ...
                    try {
                        ...
                        if ($head['Command_Id'] == 0x00000001) { //请求链接
                            ...
                            $receive_develop_no = mb_substr(trim($body['Src_Id']), $develop_len);
                            if ($body['Pk_total'] > 1) { //长短信
                                ...
                                // print_r($udh);
                                // die;
                                $pos = unpack('c', substr($Msg_Content['Msg_Content'], 3, 1)); //相同主键的短信
                                $messagetotal = unpack('c', substr($Msg_Content['Msg_Content'], 4, 1)); //拆分总条数
                                $messageindex = unpack('c', substr($Msg_Content['Msg_Content'], 5, 1)); //第几条
                                ...
                                $sendData['send_msgid'][$messageindex[1]] = $num1 . $num2;
                                $sendData['uid'] = $uid;
                                $sendData['Submit_time'] = time();
                                // print_r($pos[1]);
                                // die;
                                // $redis->rpush($redisMessageCodeSendReal, json_encode($sendData));
                                $has_message = $redis->hget($redisMessageCodeSend, $pos[1]);
                                if ($has_message) {
                                    if ($messageindex[1] == 1) {
                                        $redis->hdel($redisMessageCodeSend, $pos[1]);
                                        $redis->hset($redisMessageCodeSend, $pos[1], json_encode($sendData));
                                    } else {
                                        $has_message = json_decode($has_message, true);
                                        $has_message['send_msgid'][$messageindex[1]] = $num1 . $num2;
                                        $has_message['message'][$messageindex[1]] = $message;

                                        // print_r($has_message);
                                        if (count($has_message['message']) == $has_message['messagetotal']) {
                                            // print_r($has_message);
                                            $sendData = [];
                                            $sendData = [
                                                'mobile' => trim($mobile),
                                            ...
                                            ksort($has_message['send_msgid']);
                                            ksort($has_message['message']);
                                            foreach ($has_message['send_msgid'] as $key => $value) {
                                            ...
                                            $redis->hset($redisMessageCodeSend, $pos[1], json_encode($has_message));
                                        }
                                    }
                                } else {
                                    $redis->hset($redisMessageCodeSend, $pos[1], json_encode($sendData));
                        ...
                        if ($status == 0) { //回执
                            ...
                            if (!empty($deliver)) {
                                ...
                                $deliver_timestring = "10" . ($time + $uid * 1000 + $i);
                                ...
                                $upriver = $redis->lpop('index:message:upriver:' . $uid); //取出用户发送任务
                                if (!empty($upriver)) {
                                    $upriver = json_decode($upriver, true);
                                    $submittime = date('YmdHis', strtotime($upriver['get_time']));

                                    $code = mb_convert_encoding($upriver['message_info'], 'UCS-2', 'UTF-8');
                                    $len = strlen($code);
                                    // $deliver_timestring = time();
                                    $time = microtime(true) * 10000;
                                    $deliver_timestring = "10" . ($time + $uid * 1000 + $i);
                                    ...
                                    $deliver_num2 = substr($deliver_timestring, 8) . $this->combination($i);
                                    $deliver_bodyData = pack("N", $deliver_num1) . pack("N", $deliver_num2);
                                    $deliver_bodyData .= pack('a21', $Dest_Id . $upriver['develop_no']);
                                    $deliver_bodyData .= pack('a10', $Service_Id);
                                    $deliver_bodyData .= pack('C', 0);
                                    $deliver_bodyData .= pack('C', 0);
                                    $deliver_bodyData .= pack('C', 8); //Msg_Fmt
                                    $deliver_bodyData .= pack('a21', $upriver['mobile']);
                                    $deliver_bodyData .= pack('C', 0);
                                    // $send1 = substr($value, 0, 8);
                                    // $send2 = substr($value, 8, 8);
                                    $deliver_Msg_Content = '';
                                    $deliver_Msg_Content = pack("N", $deliver_num1) . pack("N", $deliver_num2);
                                    $deliver_Msg_Content .= pack("a" . $len, $code);
                                    $deliver_Msg_Content .= pack("a10", $submittime);
                                    $deliver_Msg_Content .= pack("a10", $submittime);
                                    $deliver_Msg_Content .= pack("a21", $upriver['mobile']);
                                    $deliver_Msg_Content .= pack("N", '');
                                    $deliver_Msg_Content_len = strlen($deliver_Msg_Content);
                                    $deliver_bodyData .= pack("C", $deliver_Msg_Content_len);
                                    $deliver_bodyData .= pack("a" . $deliver_Msg_Content_len, $deliver_Msg_Content);
    ...
    { //客户绑定ip
        if ($content == 1) { //本机测试
            return [
                'host' => "127.0.0.1", //服务商ip
                'port' => "7890", //短连接端口号   17890长连接端口号
                'Source_Addr' => "101161", //企业id  企业代码 用户名
    ...
    public function newService()
    {
        ...
        socket_bind($socket, $ip, $port) or die("error:" . socket_strerror(socket_last_error()));
    ...
    public function hanld_seesion($clien)
    {
        // socket_set_nonblock($clien); //设置非阻塞模式
        ...
        try {
            while (true) {
                ...
                $headData = socket_read($clien, 12);
                if (mb_strlen($headData) == 0) {
                ...
                $head = unpack("NTotal_Length/NCommand_Id/NSequence_Id", $headData);
                // print($ip.":".$port.">>:".json_encode($head)."\r\n");
                socket_getpeername($clien, $addr, $por);
                // print_r($head);
                if ($head['Command_Id'] == 0x00000001) { //请求链接
                    $bodyData = socket_read($clien, $head['Total_Length'] - 12);
                    $status = 0;
                    $new_bodyData = pack("C", 0); //status | 1 | Unsigned Integer |状态 0：正确 1：消息结构错  2：非法源地址  3：认证错  4：版本太高   5~ ：其他错误
                    if (strlen($bodyData) < 27) {
                        $status = 1;
                        $new_bodyData = pack("C", 1); //status | 1 | Unsigned Integer |状态 0：正确 1：消息结构错  2：非法源地址  3：认证错  4：版本太高   5~ ：其他错误
                    } else {
                        $body = unpack("a6Source_Addr/a16AuthenticatorSource/CVersion/NTimestamp", $bodyData);
                        ...
                        //账户验证
                        $user = [];
                        $user = $this->getUserAccount($body['Source_Addr']);
                        $uid = $user['uid'];
                        $Dest_Id = $user['Dest_Id']; //短信接入码 短信端口号
                        // print_r($user);
                        if (!in_array($addr, $user['bin_ip'])) {
                            $status = 2;
                            $new_bodyData = pack("C", 2); //status | 1 | Unsigned Integer |状态 0：正确 1：消息结构错  2：非法源地址  3：认证错  4：版本太高   5~ ：其他错误
                        }
                        if ($body['Version'] != 0x20) { //验证版本
                        ...
                        $AuthenticatorISMG = pack("a16", ''); //AuthenticatorISMG | 16 | Octet String | ISMG 认证码，用于鉴别 ISMG。 其值通过单向 MD5 hash 计算得出， 表示如下： AuthenticatorISMG =MD5 （Status+AuthenticatorSource+shared secret），Shared secret 由中国移动 与源地址实体事先商定， AuthenticatorSource 为源地址实体 发送给 ISMG 的对应消息 CMPP_Connect 中的值。  认证出错时，此项为空。
                        if ($status != 3) {
                            $AuthenticatorISMG = pack("a16", md5($status . $bodyData . $user['Shared_secret'], true));
                        }
                        $new_bodyData = $new_bodyData . $AuthenticatorISMG . pack("C", 0x20);
                        // // echo $new_bodyData;die;
                        $Total_Length = strlen($new_bodyData) + 12;
                        ...
                        socket_write($clien, $new_headData . $new_bodyData, $Total_Length);
                        // socket_write的作用是向socket_create的套接流写入信息，或者向socket_accept的套接流写入信息
                        if ($status != 0) {
                            socket_close($clien);
                            print("进程ID:$id == 客户端:" . $ip . ":" . $port . "已关闭连接\r\n");
                            // exit();
                            // die;
                            return false;
                        }
                    ...
                    // $contentlen = $head['Total_Length'] - 12 - 116;
                    $bodyData = socket_read($clien, 117);
                    $body = unpack("N2Msg_Id/CPk_total/CPk_number/CRegistered_Delivery/CMsg_level/a10Service_Id/CFee_UserType/a21Fee_terminal_Id/CTP_pId/CTP_udhi/CMsg_Fmt/a6Msg_src/a2FeeType/a6FeeCode/a17ValId_Time/a17At_Time/a21Src_Id/CDestUsr_tl", $bodyData);
                    $Pk_total = $body['Pk_total']; //相同 Msg_Id 的信息总条数
                    $Pk_number = $body['Pk_number']; //相同 Msg_Id 的信息总条数
                    $develop_len = strlen($Dest_Id);
                    ...
                    if ($body['Pk_total'] > 1) { //长短信
                        ...
                        $Dest_terminal_Id = 21 * $body['DestUsr_tl']; // Dest_terminal_Id接收短信的 MSISDN 号码
                        $c_length = $Dest_terminal_Id + 1;
                        $bodyData1 = socket_read($clien, $c_length);
                        $body1 = unpack("a" . $Dest_terminal_Id . "Dest_terminal_Id/CMsg_length", $bodyData1);

                        $mobile = $body1['Dest_terminal_Id'];
                        $Msg_length = $body1['Msg_length'];
                        $bodyData2 = socket_read($clien, $Msg_length);
                        //    // print_r($bodyData2);die;
                        // echo "\n";
                        $Msg_Content = unpack("a" . $Msg_length . "Msg_Content", $bodyData2);
                        $Msg_Content['Msg_Content'] = strval($Msg_Content['Msg_Content']);
                        ...
                        // $timestring = time();
                        $time = microtime(true) * 10000;
                        $timestring = "10" . ($time + $uid * 1000 + $i);
                        $back_Command_Id = 0x80000004; //发送应答
                        $num1 = substr($timestring, 0, 8);
                        $num2 = substr($timestring, 8);
                        $new_bodyData = pack("N", $num1) . pack("N", $num2);
                        $new_bodyData = $new_bodyData . pack('C', 0);
                        // $Total_Length = strlen($CMPP_SUBMIT_RESP) + 12;
                        // $RESP_headData     = pack("NNN", $Total_Length, $back_Command_Id, $head['Sequence_Id']);
                    ...
                    // print_r($head);
                    socket_close($clien);
                    exit("异常通讯包:" . json_encode($head));
                    socket_read($clien, $head['Total_Length'] - 12);
                    $new_bodyData = $new_bodyData = pack("a1", '');
                    $back_Command_Id = 0x80000008;
                    $Total_Length = strlen($new_bodyData) + 12;
                    $new_headData = pack("NNN", $Total_Length, $back_Command_Id, $head['Sequence_Id']);
                ...
                if ($status == 0) {
                    ...
                    if (!empty($deliver)) {
                        ...
                        $deliver_timestring = "10" . ($time + $uid * 1000 + $i);
                        $deliver_num1 = substr($deliver_timestring, 0, 8);
                        $deliver_num2 = substr($deliver_timestring, 8);
                        $deliver_bodyData = pack("N", $deliver_num1) . pack("N", $deliver_num2);
                        $deliver_bodyData .= pack('a21', $Dest_Id . $deliver['develop_no']);
                        $deliver_bodyData .= pack('a10', '');
                        $deliver_bodyData .= pack('C', 0);
                        $deliver_bodyData .= pack('C', 0);
                        $deliver_bodyData .= pack('C', 0); //Msg_Fmt
                        $deliver_bodyData .= pack('a21', $deliver['mobile']);
                        ...
                        if (isset($deliver['send_msgid'])) {
                            ...
                            // // echo $new_headData . $new_bodyData."\n";
                            // // echo $back_Command_Id."\n";
                            socket_write($clien, $new_headData . $deliver_bodyData, $Total_Length);
                        }
                    }
                }
