...
class ServerSocketJGZ extends Pzlife
{
                ...
                if ($headData != false) {
                    ...
                    try {
                        ...
                        if ($head['Command_Id'] == 0x00000001) { //请求链接
                            ...
                            $userEquities = Db::query("SELECT `id`,`num_balance` FROM yx_user_equities WHERE  `delete_time` = 0 AND `uid` = " . $uid . " AND `business_id` = 9");
                            if (empty($userEquities) || $userEquities[0]['num_balance'] < 0) {
                                $new_bodyData    = $new_bodyData . pack('C', 10);
                            } else {
    ...
    { //客户绑定ip 数荷
        ...
        return [
            ...
            'Source_Addr'   => "101105", //企业id  企业代码移动
            'Shared_secret' => 'Jgz123456', //网关登录密码
            'Service_Id'    => "",
            'Dest_Id'       => "10692054963", //短信接入码 短信端口号
            'Sequence_Id'   => 1,
            'SP_ID'         => "",
            'bin_ip'        => ["47.110.134.122", "127.0.0.1"], //客户端绑定IP
            'free_trial'    => 2,
            'master_num'    => 300,
            'uid'           => 115,
        ];
