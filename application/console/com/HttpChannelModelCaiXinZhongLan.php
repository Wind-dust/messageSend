...
//http 通道,通道编号10
class HttpChannelModelCaiXinZhongLan extends Pzlife
{
            ...
            while (true) {
                ...
                do {
                    ...
                    if ($send_data) {
                        ...
                        if (empty($send_task)) {
                            ...
                            $send_template_id[$send_data['mar_task_id']] = $send_data['template_id'];
                        ...
                        foreach ($send_num as $send_taskid => $num) {
                            ...
                            if (count($new_num) >= 1000) { //超出500条做一次提交
                                //单条测试
                                // print_r($send_content[$send_taskid]);die;
                                ...
                                $data    = [
                                    'Mobile' => join(',',$new_num),
                                    'Mmsid'  =>  $send_template_id[$send_taskid],
                                    'sToken'  => $sToken,
                                ];
                                // echo time();
                ...
                if (!empty($send_num)) {
                    foreach ($send_num as $send_taskid => $num) {
                        ...
                        // print_r($send_data['mar_task_id']);die;
                        $sToken  = '7100455520709585';
                        if (!empty($send_develop_code[$send_taskid])) {
                            $sToken = $sToken. $send_develop_code[$send_taskid];
                        }
