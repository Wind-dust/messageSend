...
class CmppRongHeSanWangBusiness extends Pzlife
{
    ...
    public function content($content)
    {
        return [
            ...
            'Source_Addr'   => "484036", //企业id  企业代码
            'Shared_secret' => '576200', //网关登录密码
            'Service_Id'    => "484036", //业务代码
            'template_id'   => "", //模板id
                        ...
                        try {
                            ...
                            while (true) {
                                ...
                                if ($headData != false) {
                                    ...
                                    if (strlen($headData) < 12) {
                                        $headData .= socket_read($socket, 12 - strlen($headData));
                                    }
                                    $head = unpack("NTotal_Length/NCommand_Id/NSequence_Id", $headData);
                                    $bodyData = socket_read($socket, $head['Total_Length'] - 12);
                                    if ($head['Command_Id'] == 0x80000001) {
