...
//http 通道,通道编号10
class HttpChannelCaiXinZhongLan extends Pzlife {
            ...
            while (true) {
                ...
                do {
                    ...
                    if ($send_data) {
                        ...
                        foreach ($send_num as $send_taskid => $num) {
                            ...
                            if (count($new_num) >= 1000) { //超出500条做一次提交
                                //单条测试
                                $html = '';
                            // $html .= "<?xml version='1.0' encoding='utf-8'>";
                            $html .= "<mms>";
                            $html .= "<subject>" . $send_title[$send_taskid] . "</subject>";

                            $html .= '<pages>';
                            foreach ($send_content[$send_taskid] as $key => $value) {
                                $html .= '<page dur="50">';
                            ...
                            $mmsPack = $html;
                            $sToken  = '7100455520709585';
                            if (!empty($send_develop_code[$send_taskid])) {
                                $sToken = $sToken. $send_develop_code[$send_taskid];
                            }
                            $data    = [];
                            $data    = [
                                'mmsPack' => $mmsPack,
                            ...
                            // echo time();
                            //创建模板
                            $res = $this->sendRequestXwww('http://www.wemediacn.net/webservice/mmsservice.asmx/PostMMSMessage', $data);
                            // print_r($res);die;
                ...
                if (!empty($send_num)) {
                    foreach ($send_num as $send_taskid => $num) {
                        ...
                        // print_r($send_content[$send_taskid]);
