...
//http 通道,通道编号10
class HttpChannelModelSupMessageAiQi extends Pzlife
{
    ...
    public function content($content = 133)
    {
        return [
            ...
            'send_var_api' => 'http://api.qxt365.cn/Recv_center', //新模板变量发送地址
                ...
                if (!empty($send_data)) {
                    ...
                    if ($j > 2000) {
                        foreach ($send_task as $key => $value) {
                            $request_data = [];
                            $CpName = $user_info['CpName']; //appid由企业彩信平台提供 是
                            ...
                            $request_data = [
                                ...
                                'DesMobile' => join(',', $value['phones']),
                                'ExtCode' => $value['develop_no'],
                            ];
                            # code...
                            ...
                            $res = sendRequest($user_info['send_var_api'], 'post', $request_data);
                            $result = json_decode($res, true);
                            if (isset($result['code']) && $result['code'] == 0) {
                                $redis->hset('index:meassage:code:back_taskno:aiqi', $result['smsid'], $key);
                                unset($send_task[$key]);
                                $j = 1;
                            } else {
                    ...
                    foreach ($send_task as $key => $value) {
                        ...
                        if (isset($result['code']) && $result['code'] == 0) {
                            ...
                            $this->writeToRobot($content, $res, '艾麒视频短信通道');
                            exit();
                        }
                    }
                }
