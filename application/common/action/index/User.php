...
class User extends CommonIndex
{
    ...
    public function getUserSubmitTask($page, $pageNum, $conId)
    {
        ...
        $result = DbAdministrator::getUserSendTask(['uid' => $uid], '*', false, ['id' => 'desc'], $offset . ',' . $pageNum);
    ...
    public function getUserSubmitTaskInfo($page, $pageNum, $ConId, $id)
    {
        ...
        $task_log = DbAdministrator::getUserSendTaskLog(['task_no' => $task['task_no']], '*', false,  ['id' => 'desc'], $offset . ',' . $pageNum);
    ...
    public function getUserBusinessSubmitTask($page, $pageNum, $conId)
    {
        ...
        $offset = ($page - 1) * $pageNum;
        $result = DbAdministrator::getUserSendCodeTask(['uid' => $uid], '*', false,  ['id' => 'desc'], $offset . ',' . $pageNum);
    ...
    public function getUserBusinessSubmitTaskInfo($page, $pageNum, $ConId, $id)
    {
        ...
        $task_log = DbAdministrator::getUserSendCodeTaskLog(['task_no' => $task['task_no']], '*', false,  ['id' => 'desc'], $offset . ',' . $pageNum);
    ...
    public function getUserMultimediaMessageTask($page, $pageNum, $ConId, $start_time = '', $end_time = '', $title = '')
    {
        ...
        $offset = ($page - 1) * $pageNum;
        $result = DbSendMessage::getUserMultimediaMessage($where, '*', false,  ['id' => 'desc'], $offset . ',' . $pageNum);
        if (empty($result)) {
    ...
    public function getUserMultimediaMessageTaskInfo($page, $pageNum, $ConId, $id)
    {
        ...
        $task_log = DbSendMessage::getUserMultimediaMessageLog(['task_no' => $task['task_no']], '*', false,  ['id' => 'desc'], $offset . ',' . $pageNum);
    ...
    public function setUserBalance($ConId, $balance, $new_mobile, $business_id){
        $uid = $this->getUidByConId($ConId);
        if (empty($uid)) { //用户不存在
            return ['code' => '3003'];
        }
        ...
        $equitise = DbAdministrator::getUserEquities(['business_id' => $business_id, 'uid' => $uid], '*', true);
        if (empty($equitise)) {
            return ['code' => '3002'];
        }
        Db::startTrans();
        try {
            // DbUser::updateUser(['logo' => $image, 'businesslicense' => $bimage], $uid);
            DbAdministrator::editUserEquities(['balance' => $balance, 'mobile' => join(',',$new_mobile)], $equitise['id']);
            Db::commit();
            return ['code' => '200'];
        } catch (\Exception $e) {
            Db::rollback();
