...
class Upload extends CommonIndex
{
     ...
     * @return array
     */
    public function uploadVideo($appid, $appkey, $fileInfo)
    {
        /* 文件名重命名 */
        $user = DbUser::getUserOne(['appid' => $appid], 'id,appkey,user_type,user_status,reservation_service,free_trial', true);
        if (empty($user)) {
        ...
        $filename    = date('Ymd') . '/' . $this->videoupload->getNewName($fileInfo['name']);
        $uploadvideo = $this->videoupload->uploadFile($fileInfo['tmp_name'], $filename);
        if ($uploadvideo) { //上传成功
            $result = DbVideo::saveLogVideo($filename);
            if ($result) {
                return ['code' => '200', 'file_path' => Config::get('qiniu.videodomain') . '/' . $filename];
            } else {
                $this->delVedio($filename); //删除上传的图片
            }
        }
        return ['code' => '3003']; //上传失败
    }
     ...
     * @param $filenameArr
     */
    private function delVedio($filenameArr)
    {
        if (!is_array($filenameArr)) {
            $this->videoupload->deleteImage($filenameArr); //删除上传的图片
        } else {
            foreach ($filenameArr as $v) {
                $this->videoupload->deleteImage($v); //删除上传的图片
            }
        }
    }
}
