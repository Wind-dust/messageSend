...
class SupAdminController extends Controller {
    ...
    protected function isLogin() {
        $this->headers();
        $supConId = trim($this->request->param('sup_con_id'));
        if (!empty($supConId) && strlen($supConId) == 32) {
            $res = $this->app->user->isLogin($supConId);//判断是否登录
            if ($res['code'] == '200') {
                return;
            }
